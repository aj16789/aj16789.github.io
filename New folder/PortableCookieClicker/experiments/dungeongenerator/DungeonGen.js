...
var DungeonGen=function()
{
		...
		else
		{
			...
			else
			{
				...
				if (room.tiles.length>0 && room.freeTiles>0)//we got a decent room
				{
					...
					if (this.isFloor(door[0]+side[0],door[1]+side[1])!=-1) this.freeTiles.splice(this.isFloor(door[0]+side[0],door[1]+side[1]),1);
					if (this.isFloor(door[0]-side[0],door[1]-side[1])!=-1) this.freeTiles.splice(this.isFloor(door[0]-side[0],door[1]-side[1]),1);
	...
	this.Map.prototype.drawDetailed=function()
	{
		...
		for (var y=0;y<this.h;y++){for (var x=0;x<this.w;x++){
				...
				str+='<div style="opacity:'+opacity+';width:'+size+'px;height:'+size+'px;position:absolute;left:'+(x*size)+'px;top:'+(y*size)+'px;display:block;padding:0px;margin:0px;background:#'+colors[this.data[x][y][0]]+' url(dungeonTiles.png) '+(-pic[0]*16)+'px '+(-pic[1]*16)+'px;color:#999;" title="'+title+'"></div>';
			}
			str+='<br>';
		}
